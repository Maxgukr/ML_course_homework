...
def simple_smo(dataset, labels, C, max_iter):
    ...
    while it < max_iter:
        ...
        for i in range(m):
            ...
            fx_i = f(x_i)
            E_i = fx_i - y_i
            #if E_i*labels[i] < -tol and alphas[i] < C or E_i*labels[i] > tol and alphas[i] > 0:
            j = select_j(i, m)
...
def main():
    ...
    dataset, labels = make_classification(n_samples=200, n_features=2, n_redundant=0, n_informative=2,
    ...
    a1, a2 = w
    y1, y2 = (-(b-0.5) - a1*x1)/a2, (-(b-0.5) - a1*x2)/a2
    ax.plot([x1, x2], [y1, y2])
    # 绘制支持向量
    ''''
